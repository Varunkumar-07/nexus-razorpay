...
### 11 — Groq daily token quota exhausted during Phase 3 (legacy static UI removal) regression run
- **Date/Time:** 2026-09-02
- **Phase:** Post-Phase-12 cleanup — Phase 3 of the post-audit remediation plan (retiring the legacy `app/web_chat/static/` UI and its mount in `app/agent_api/main.py`)
- **Component:** External — Groq's `openai/gpt-oss-120b` model, surfaced through `app/reasoning/agent.py`'s tool-calling loop, same as Entries 8/9/10. No NEXUS code is at fault.
- **What broke (symptom):** Running `scripts/test_web_chat.py` after removing the legacy static UI mount, Case 1 (the LLM-dependent Scenario A flow) failed twice in a row: first a 30s `requests.ReadTimeout` on `POST /chat/message`, then on immediate retry a clean 200 response but with the chat adapter's own graceful-degradation reply ("Sorry, NEXUS is temporarily at capacity...", `app/web_chat/routes.py:83-87`), so the test's `state == "AWAITING_QUANTITY"` assertion failed. A direct probe (`app.reasoning.agent.recommend()` called standalone, bypassing the test harness entirely) confirmed the actual cause: `groq.RateLimitError` / HTTP 429 — `"Rate limit reached for model openai/gpt-oss-120b ... tokens per day (TPD): Limit 200000, Used 199613, Requested 1044. Please try again in 4m43.824s."`
- **What we were trying to do:** Verifying Phase 3 (legacy static UI retirement: removed the `StaticFiles` mount and `/products` route from `app/agent_api/main.py`, deleted `app/web_chat/pages.py` and `app/web_chat/static/`, added a minimal JSON root route) didn't regress the Chat Adapter's HTTP wrapper, by re-running the full `scripts/test_web_chat.py` suite.
- **Root cause:** Not a NEXUS bug and not related to the Phase 3 changes at all — Phase 3 touches only `app/agent_api/main.py` (routing/mounting), `app/web_chat/pages.py` (deleted), and `app/web_chat/static/` (deleted), none of which are on the `POST /chat/message` -> `ChatSession` -> `recommend()` -> Groq call path. This is the same daily-quota exhaustion class already documented in Entry 10 (which noted a 199,075-199,685/200,000 TPD saturated band 8 days earlier); the account's shared 200,000 TPD budget was simply used up again by the time of this run.
- **How we solved it:** No code change (there is nothing to fix — this is an external quota, and the Phase 3 diff doesn't touch the reasoning path). Confirmed the routes Phase 3 actually changes are unaffected by testing them directly, independent of the quota-blocked LLM path: `GET /` (now returns the JSON API-root response, not the old static UI), `GET /products` (now 404, route removed), `GET /docs`, `GET /catalog/all`, and `GET /metrics/summary` all verified via direct `curl` against a locally running server — all returned the expected status codes. Ran a bounded background retry of `scripts/test_web_chat.py` (8 attempts, 30s apart, ~6 minutes total, well past Groq's own "try again in 4m43s" estimate) to give the quota window a real chance to clear.
- **Verification:** All 8 retry attempts failed identically — Case 1's first LLM call either read-timed-out (attempt 1) or returned the Chat Adapter's own graceful-degradation reply ("Sorry, NEXUS is temporarily at capacity...", `app/web_chat/routes.py:83-87`) for the rest, meaning the 200,000 TPD quota did not free up in that window. This matches Entry 10's exact prior pattern ("oscillating in a saturated band for 45+ minutes... evidently from other concurrent activity on the same account, not from this work") — the quota is apparently being consumed by other concurrent use of the same Groq account, not by NEXUS's own test traffic, and a single ~6 minute wait wasn't enough this time either. In lieu of an end-to-end `test_web_chat.py` pass (blocked), Phase 3's actual changes were verified directly and independently of Groq: `GET /` (JSON API-root, not the old UI), `GET /products` (404, route removed), `GET /docs`, `GET /catalog/all`, `GET /metrics/summary` — all correct. `app/web_chat/routes.py`'s own request handling (session creation, the LLM call, and its graceful-degradation path) is demonstrably still working end-to-end; only the underlying Groq call itself is quota-blocked, which is an existing, already-logged external condition (Entry 10), not something Phase 3 introduced.
- **Tools/resources used:** Direct standalone call to `app.reasoning.agent.recommend()` to isolate the failure from the test harness and confirm the exact Groq error; `curl` against a locally started `uvicorn` instance to verify the non-LLM routes Phase 3 actually touches; a background bounded retry loop (8 attempts / 30s apart) to check whether the quota cleared within Groq's own estimated window.
- **Time lost:** ~15 minutes (diagnosis + direct probe + the 6-minute bounded retry loop).
- **Status:** Open — blocked on external Groq daily-quota headroom, same underlying condition as Entry 10, not a Phase 3 regression. Phase 3's own changes are independently confirmed correct via direct route checks; the full `test_web_chat.py` LLM-dependent case should be re-run once quota is available, per Entry 10's same open follow-up.
